...
!===================================================================================================================================
  ...
  IF(filename(ppos:ppos+3).NE.'.nc')THEN
    WRITE(UNIT_stdOut,'(4X,A)')'VMEC READ WOUT FILE "'//TRIM(fileName)//'" in NEMEC format ...'
    CALL ReadNEMEC(fileName,"formatted",ok)
...
!===================================================================================================================================
SUBROUTINE ReadNEMEC(fileName,format_type,ok)
...
!===================================================================================================================================
  ...
  ELSE
    ...
    WRITE(UNIT_stdOut,*)'format_type:        ', trim(format_type)
    WRITE(UNIT_stdOut,*) '****** no valid key-word: stop *****'
    ok=-1
    RETURN
  END IF
  
  ! open equilibrium file
  open(inUnit,file=trim(filename),form=trim(format_type), &
...
! --- full mesh
  ...
  STOP "VMEC READIN FROM NEMEC NOT FINISHED YET!!!"
...
!    ioError = ioError + NF_INQ_DIMLEN(ncid, id, nFluxVMEC)
    ...
    IF (ioError .NE. 0) CALL abort(__STAMP__,&
                              'VMEC READIN: problem reading mnmax' )
    !! get mnmax_nyq
