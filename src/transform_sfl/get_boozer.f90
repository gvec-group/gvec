...
INTERFACE Get_Boozer_sinterp
  MODULE PROCEDURE Get_Boozer_sinterp
END INTERFACE

INTERFACE Get_Boozer_sproject
  MODULE PROCEDURE Get_Boozer_sproject
END INTERFACE
...
!===================================================================================================================================
SUBROUTINE Get_Boozer_sinterp(mn_max,fac_nyq,sgrid_in,G_base_out,Gthet,GZ)
! MODULES
...
!$OMP   SCHEDULE(STATIC) DEFAULT(SHARED) PRIVATE(iMode)
  DO iMode=1,modes
    ...
    CALL G_base_out%s%applyBCtoDOF(GZ(:,iMode),(/BCtype_axis(G_base_out%f%zero_odd_even(iMode)),BC_TYPE_OPEN/)  &
                                              ,(/0.,0./))
    CALL G_base_out%s%applyBCtoDOF(Gthet(:,iMode),(/BCtype_axis(G_base_out%f%zero_odd_even(iMode)),BC_TYPE_OPEN/)  &
                                              ,(/0.,0./))
  END DO
!$OMP END PARALLEL DO 
...
!!=> dG/dzeta  = (B_zeta  - Ipol - Itor*dlambda/dzeta  ) / (Itor*iota+Ipol)
!===================================================================================================================================
SUBROUTINE Get_Boozer_sproject(mn_max,fac_nyq,sgrid_in,G_base_out,Gthet,GZ)
! MODULES
USE MODgvec_Globals,ONLY: UNIT_stdOut,TWOPI,PI,ProgressBar
USE MODgvec_LinAlg
USE MODgvec_base   ,ONLY: t_base,base_new
...
!'_sin_   ', &
  ...
  modes        = G_base_out%f%modes  !number of modes in output
  nBase        = G_base_out%s%nBase  !number of radial points in output
  nGP          = G_base_out%s%nGP
  nElems       = G_base_out%s%grid%nElems
  degGP        = G_base_out%s%degGP
  deg          = G_base_out%s%deg
  dthet_dzeta  = G_base_out%f%d_thet*G_base_out%f%d_zeta !integration weights
...
!$OMP END PARALLEL DO 
    ...
    !interpolate G and compute Gthet=iota*G+lambda
    ft_IP(:) =LA_IP(:)+iota_int*G_base_out%f%evalDOF_IP(0,GZ_GP(iGP,:))
    !project onto Gthet base
    CALL G_base_out%f%projectIPtoDOF(.FALSE.,1.0_wp, 0,ft_IP(:),Gthet_GP(iGP,:))
    DO iMode=1,modes
      Gthet_GP(iGP,iMode)=Gthet_GP(iGP,iMode)*dthet_dzeta*G_base_out%f%snorm_base(iMode)
    END DO
    GZ_GP(   iGP,:)=G_base_out%s%w_GP(iGP)*GZ_GP(   iGP,:)
    Gthet_GP(iGP,:)=G_base_out%s%w_GP(iGP)*Gthet_GP(iGP,:)
...
!$OMP   SCHEDULE(STATIC) DEFAULT(SHARED) PRIVATE(iMode,iElem,iGP,iBase)
  DO iMode=1,modes
    ...
    DO iElem=1,nElems
      ...
      ibase=G_base_out%s%base_offset(iElem)
      GZ(iBase:iBase+deg,iMode) = GZ(iBase:iBase+deg,iMode) & 
                                    + MATMUL(GZ_GP(iGP:iGP+degGP,iMode),G_base_out%s%base_GP(0:degGP,0:deg,iElem))
      Gthet(iBase:iBase+deg,iMode) = Gthet(iBase:iBase+deg,iMode) &
                                    + MATMUL(Gthet_GP(iGP:iGP+degGP,iMode),G_base_out%s%base_GP(0:degGP,0:deg,iElem))
    END DO !iElem
    CALL G_base_out%s%mass%solve_inplace(1,GZ(:,iMode))
    CALL G_base_out%s%mass%solve_inplace(1,Gthet(:,iMode))
    CALL G_base_out%s%applyBCtoDOF(GZ(:,iMode),(/BCtype_axis(G_base_out%f%zero_odd_even(iMode)),BC_TYPE_OPEN/)  &
                                              ,(/0.,0./))
    CALL G_base_out%s%applyBCtoDOF(Gthet(:,iMode),(/BCtype_axis(G_base_out%f%zero_odd_even(iMode)),BC_TYPE_OPEN/)  &
                                              ,(/0.,0./))
