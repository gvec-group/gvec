...
! Copyright (C) 2019 - 2019  Erika Strumberger

! This file is a modified version from HOPR (github.com/fhindenlang/hopr) and uses the readNEMEC routine from Erika Strumberger.
!
...
! LOCAL VARIABLES
  INTEGER :: ppos,ok
!===================================================================================================================================
  ppos=SCAN(fileName,".",back=.TRUE.)
  IF(filename(ppos:ppos+3).EQ.'.nc')THEN
#if NETCDF
...
#endif
  ELSE
    SWRITE(UNIT_stdOut,'(4X,A)')'VMEC READ WOUT FILE "'//TRIM(fileName)//'" in NEMEC format ...'
    CALL ReadNEMEC(fileName,ok)
    IF(ok.NE.0) CALL abort(__STAMP__,&
        "Problems with VMEC readin from NEMEC format")
  END IF !.nc file ending
...
!===================================================================================================================================
SUBROUTINE ReadNEMEC(fileName,ok)
...
! --- translate to GVEC datastructure  
  ...
  FUNCTION checkFormat(fName)
    ...
    INTEGER   :: fId, stat,i
    CHARACTER(LEN=100) :: c
    INTEGER   :: checkFormat !output: -1: problem, 0:unformatted, 1: formatted
    LOGICAL   :: isFormatted
    !
    stat = 0
    isFormatted = .TRUE. !assume formatted
    checkFormat = 1 
    c=" "
    OPEN(newunit=fId,file=fName,status='old',form='unformatted',iostat=stat)
    IF(stat.NE.0) THEN
      checkFormat=-1 ;  RETURN
    END IF
    ! I assume that it fails only after 10 characters
    READ(fId, iostat=stat)c
    IF(stat.GT.0) THEN
      checkFormat=-2 !;  RETURN
    END IF
    DO i=1,100
        isFormatted = isFormatted.AND.( iachar(c(i:i))<=127 )
    END DO
    CLOSE(fId)
    IF(.NOT.isFormatted) checkFormat=0    
