...
def gframe_to_vtk(
    ...
    try:
        ...
        logger.warning(f"axis group not found in {file} : {e}, cannot continue")
        return
    ...
    if filetype == "vts":
        vtk.ev2vtk(f"{prefix}_axis", xr_axis)
    elif filetype == "nc":
    ...
    if box_axis is not None:
        ...
        if filetype == "vts":
            vtk.ev2vtk(f"{prefix}_box_axis", xr_box)
        elif filetype == "nc":
    ...
    try:
        ...
        logger.warning(
            f"boundary group not found in {file}. boundary file will not be written. {e}."
        )
        return
    ...
    if filetype == "vts":
        vtk.ev2vtk(f"{prefix}_boundary", xr_bnd)
    elif filetype == "nc":
