...
def boundary_generator(case: str, nfp, X1_00=1.0, a0=0.5, ellipticity=0.4, helix_r=0.5):
    ...
    To evaluate the boundary at the given (theta, zeta) points, use
    `evaluate_boundary(theta,zeta,boundary_generator(name,nfp))`
    Args:
        case (str): The name of the boundary:
        ...
        X1_00 : =major radius if $X^1=R$
        a0    : =minor radius scale
        ellipticity : =ellipticity of the cross section
    Returns:
        parameter dictionary describing $X^1$ and $X^2$
    """
    assert case in ["ellip_cyl", "ellip_cyl_breathe"]
    params = {"nfp": nfp}
    if case == "ellip_cyl":
