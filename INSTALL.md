...
and the environment variable `$HOMEBREW_PREFIX` needs also to be set.
If not, issue the following commands, which correspond to the default homebrew path:
...
If you installed homebrew using a different prefix, you need to adapt the commands above accordingly.
...
```bash
srun --nodes=1 --ntasks-per-node=<nr.MPItasks> --cpus-per-task=$OMP_NUM_THREADS <nr.cores> -p interactive --time=TIME_LESS_THAN_2HOURS --mem=MEMORY_LESS_THAN_32G build/bin/gvec ini/performance/parameter_tiagoMPI.ini
```
...
1. Inside that directory execute
   ...
   * In the main `CMakeList.txt` file, some pre-defined setups (library paths) for different architectures are controlled by setting the  `CMAKE_HOSTNAME` to `cobra`/`raven`/`mac_brew`/`mac_ports`/`tokp`/...
